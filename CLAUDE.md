...
- `src/data/*.ts`：城市布局、物品/商店、产品化学、顾客定义、第一章对话卡（`story.ts`，每步一次，说话人/地点必须存在于数据文件）。序章节奏由 `systems/StorySystem.ts` 从状态推导（不落盘），`ordersAllowed` 决定呼机何时放单。电台文案里出现的商家必须存在于 `city.ts`。
