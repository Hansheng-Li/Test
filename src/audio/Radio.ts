...
export class Radio {
  ...
  private nextLine(): string {
    ...
    if (c && c.eventId === 'curfew' && Math.random() < 0.5 && this.lastText !== 'event') {
      this.lastText = 'event';
      return `${st.dj}: ` + (st.synth ? 'Curfew tonight. Extra patrols, fewer shadows. Stay sharp out there.' : 'District 3 has declared a curfew from eight until six. Extra officers downtown and a second cruiser on the beach, folks. Get home safe.');
    }
