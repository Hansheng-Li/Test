...
export class PagerUI extends Panel {
  ...
  render(): void {
    ...
    for (const o of pending) {
      ...
      head.innerHTML = faceImg(c.id, c.shirt, 'avatar big');
    ...
    for (const o of active) {
      ...
      card.innerHTML = `<div class="order-head">${faceImg(c.id, c.shirt)}<div style="flex:1;min-width:0"><b>${c.name}</b> · ${o.qty}x ${request} · $${o.price}<br/>` +
        ...
        (o.status === 'runner'
          ...
          : have ? `<span style="color:#7dff9a">${t('YOU ARE CARRYING THE GOODS')}</span>` : `<span style="color:#ffb3c1">${t('YOU DO NOT HAVE {n}x {what} YET', { n: o.qty, what: request })}</span>`) + '</div></div>';
      const actions = document.createElement('div');
      actions.className = 'actions';
