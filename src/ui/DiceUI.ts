...
export class DiceUI extends Panel {
  ...
  render(): void {
    ...
    body.innerHTML = `<div class="desc" style="margin-bottom:8px">${t('HIGH wins on 8 or more, LOW on 6 or less, 7 is the house. Even money; snake eyes on LOW pays triple. Every throw draws a little heat.')}</div>
      <div class="row"><span class="name">${t('Cash {cash}', { cash: `<b style="color:#7dff9a">$${Math.floor(st.cash)}</b>` })}</span><span class="price">${t('tonight: {net} · lifetime {total}', { net: `<b style="color:${this.sessionNet >= 0 ? '#7dff9a' : '#ff8fa3'}">${money(this.sessionNet)}</b>`, total: money(st.stats.diceNet ?? 0) })}</span></div>
      ...
      <div style="text-align:center;margin:6px 0 10px;color:#ffe9a8;min-height:18px">${this.last ? `${this.last.sum} · ${tn(this.last.line)} (${money(this.last.net)})` : t('Pick a bet, call it.')}</div>`;
    ...
    for (const pick of ['high', 'low'] as DicePick[]) {
      const btn = document.createElement('button');
      btn.className = 'cyan';
      btn.textContent = t(pick === 'high' ? 'CALL HIGH (8+)' : 'CALL LOW (6-)');
